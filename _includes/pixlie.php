<?php
	...
	*/
	...
	clearstatcache();
	@ini_set("max_execution_time", 300); // workaround für die Nichtanzeige einiger Bilder ???
	/*
		...
		------------------------------------------------------
			...
			- 8.3 Pruefen ob Datei schon im Cache liegt
				...
				- 8.3.4 Berechne Bilder nach dem Typ "fixed"
	 ...
	 **************************************** 1.1 Bildgroessen *****************************************
		 ...
		 Modus "fixed":
		    Wird dieser Bildberechnungstyp verwendet behält Pixlie den vorgebenen Wert für eine Seite fest ein
			 und skaliert die andere Seite entsprechend.
			 'f' => array('type'=>'fixed', 'size'=>'500, h' [ h = height | w = width ]
			 Hierbei wird die Höhe auf 500px festgehalten und die Breite entsprechend nachberechnet

		 Benutzung 'overwrite_cache':
			 Zum Testen ist es sinnvoll 'overwrite_cache' auf 'true' zu stellen. Bei jedem Seitenaufruf wird der
			 Cache wieder mit neuen Daten ueberschrieben! Diese Option ist unter 1.3 (Sonstige Einstellungen)
			 zu finden.
			 HINWEIS: DASS DER CACHE GENUTZT WIRD, MUSS DIESE EINSTELLUNG IM LIVE-BETRIEB WIEDER AUF 'false' STEHEN!
	...
	$pixlie_imageconfig = array(
		'x' => array('type'=>'cut','size'=>'80,80'), 		//Beispiel fuer Zugeschnittenes Bild mit 80 x 80 Pixel
		'y' => array('type'=>'uncut','size'=>'600'),			//Beispiel fuer ein in der Groesse angepasstes Bild mit max 600 Pixel
		'f' => array('type'=>'fixed','size'=>'550,h'),		//Beispiel fuer einen Zuschnitt bei gleicher Hoehe von 550 fuer alle Bilder (Breite wird entsprechend berechnet)
		'z' => array('type'=>'bestfit','size'=>'900,600') 	//Beispiel fuer ein in der Groesse angepasstes Bild mit max 900 x 600 Pixel, wobei die Höhe dem zweiten Wert angepasst wird. Hochformat = Höhe von Querformat. Möglich ist auch nur eine Größenangabe.	
	);
 ...
 */
	...
	$pixlie_userconfig = array(
		...
		'dir_upload'				=> dirname(__FILE__).'/upload'.((isset($manual_subpath)&&$manual_subpath!='')?'/'.$manual_subpath:((isset($_GET[$manual_subpath_var])&&$_GET[$manual_subpath_var]!='')?'/'.$_GET[$manual_subpath_var]:'')),		//Pfad zum Upload Verzeichnis
		'pic_image_quality'			=> 90,			//Bildqualitaet: 0 = schlechteste , 100 = beste (gilt nur fuer JPG, Kompression von PNG wird entsprechend umgerechnet, GIF ohne Qualitaetsangabe)
		'autoscale_quality'			=> false,		//Falls berechnetes Bild groesser als Originalbild Qualitaet soweit herabstufen, bis Zieldatei kleiner ist
		'show_metadata_iptc'		=> true,		//IPTC-Ausgabe: true = an, false = aus
		'show_metadata_exif'		=> true,		//EXIF-Ausgabe: true = an, false = aus
		'png_support'				=> false,		//aktiviert die Verarbeitung von PNG-Dateien [ true | false ]
		'gif_support'				=> false,		//aktiviert die Verarbeitung von GIF-Dateien [ true | false ]
		'pic_exclude'				=> array(),  	//Bilder ausschließen die nicht angezeigt werden sollen, wirkt sich auf alle gleichnamigen Bilder aus
		...
		'short_relpath_create'		=> false,		//bestimmt den Relpath nur anhand des Hosts im Pfad, falls normaler Relpath falsch berechnet wird (z.B.Stratokunden) [ true | false ] (Standard: false)
		'relative_path' 			=> '',			//sollten die automatisch generierten Pfade nicht stimmen, trage hier den relativen Pfad ohne abschließenden Slash zur pixlie.php ein (z.B. '/pixlie')
		'overwrite_cache'			=> false,		//Cache bei jedem Aufruf neu erstellen: true = an, false = aus
		'suppress_warnings'			=> false		//Fehlermeldungen unterdrücken [true | false] Standard: false (bei true wird die Meldung nur angezeigt, aber die Verarbeitung nicht abgebrochen. KANN ZU UNGEWÜNSCHTEM VERHALTEN FÜHREN)
	...
	$pixlie_sysconfig = array(
		...
		'manual_subpath_var' => $manual_subpath_var,		//manueller Pfad. Wird automatisch übernommen.
		'png_Quality' => round(abs(($pixlie_userconfig['pic_image_quality'] - 100) / 11.111111))
	...
	if (!function_exists('check_picture_type')) {
		function check_picture_type($pixlie_pic_type, $type)
		{
			$check = false;
			switch ($type) 
			{
				case 'jpg': 
					if (($pixlie_pic_type == 'jpg') || ($pixlie_pic_type == 'jpeg') || ($pixlie_pic_type == 'JPG') || ($pixlie_pic_type == 'JPEG'))
						$check = true;
					else
						$check = false;
				break;
				case 'png':
					if (($pixlie_pic_type == 'png') || ($pixlie_pic_type == 'PNG'))
						$check = true;
					else
						$check = false;
				break;
				case 'gif':
				if (($pixlie_pic_type == 'gif') || ($pixlie_pic_type == 'GIF'))
						$check = true;
					else
						$check = false;
				break;
	...
	if (!function_exists('check_picture_ending')) {
		function check_picture_ending($path, $pixlie_config)
		{
			$check = false;
			if((preg_match('/(.jpg)$/i', $path)) ||
			   (preg_match('/(.jpeg)$/i', $path)) ||
			   (preg_match('/(.JPG)$/i', $path)) ||
			   (preg_match('/(.JPEG)$/i', $path)) ||
			  ((preg_match('/(.gif)$/i', $path)) && $pixlie_config['gif_support']) ||
			  ((preg_match('/(.GIF)$/i', $path)) && $pixlie_config['gif_support']) ||
			  ((preg_match('/(.PNG)$/i', $path)) && $pixlie_config['png_support']) ||
			  ((preg_match('/(.png)$/i', $path)) && $pixlie_config['png_support']))
				$check = true;
			else
				$check = false;
	...
	if (!function_exists('create_data')) {
		function create_data($pixlie_pic_type, $path)
		{
			$new_img = "";
			if (check_picture_type($pixlie_pic_type, "png")) {
				$new_img = imagecreatefrompng($path);
			}
			elseif (check_picture_type($pixlie_pic_type, "jpg")) {
				$new_img = imagecreatefromjpeg($path);
			}
			elseif (check_picture_type($pixlie_pic_type, "gif")) {
	...
	$pixlie_lang = array(
		...
		'exif_function_not_exist' 	=> "Die Funktion <b>exif_read_data</b> konnte nicht gefunden werden. Bitte <b>show_metadata_exif</b> in der Pixlie-Konfiguration auf <b>'false'</b> stellen.",
		'render_missing_option'		=> "Unzureichende Angaben beim Bildberechnungstyp",
		'invalid_datatype'			=> "ERROR. Kein passender Datentyp",
		'path_not_exist'			=> "Der Pfad zur Pixlie konnte nicht ermittelt werden"
	);
	...
	if(function_exists('pixlie_error')==false){
		function pixlie_error($msg){
			...
			if ($pixlie_config['response_type'] == 'json') {
				...
				if(!$pixlie_config['suppress_warnings'])
					die($msg);
				else
					echo($msg);
			}
...
//			(preg_match("/\([^>]*\"?[^)]*\)/", $pixlie_query))			|| // auskommentiert, um Dateinamen mit Klammern zu erlauben
	 ...
	 ************************************************************************************************/
	if (check_picture_ending($pixlie_query, $pixlie_config)) {
	...
	/************************ 8.2 Cache Dateinamen und Dateipfade generieren ************************/
		...
		$pixlie_path_explode = explode(".", basename($pixlie_item_path));
		$pixlie_pic_type = $pixlie_path_explode[count($pixlie_path_explode)-1];
		$pixlie_cache_name = md5($pixlie_query).'_'.$pixlie_item_picsize.".".$pixlie_pic_type;
	...
	/*************************** 8.3 Pruefen ob Datei schon im Cache liegt ***************************/
		...
		if(file_exists($pixlie_item_path)){
			if((file_exists($pixlie_cache_path) == false) || ($pixlie_config['overwrite_cache'])){
				$pixlie_old_size = filesize($pixlie_item_path);
				$old_img = create_data($pixlie_pic_type, $pixlie_item_path);
				$old_img_x = imagesx($old_img);
				$old_img_y = imagesy($old_img);
				$quality_round = 0;
				$pic_sizes = array();
				do {
				if ($quality_round > 0) {
					$pixlie_config['pic_image_quality'] -= 1;
					$pixlie_config['png_Quality'] = round(abs(($pixlie_userconfig['pic_image_quality'] - 100) / 11.111111));
				}
	...
	/*************************** 8.3.1 Berechne Bilder nach dem Typ "cut" ***************************/
					...
					case 'cut':
						...
						$src_img = create_data($pixlie_pic_type, $pixlie_item_path);
						...
						if (check_picture_type($pixlie_pic_type, "png")) imagealphablending($dst_img, false);
						imagecopyresampled($dst_img,$src_img,0,0,$src_y,$src_x,$pixlie_render_size[0],$pixlie_render_size[1],$src_w,$src_h);
						if (check_picture_type($pixlie_pic_type, "png")) imagesavealpha($dst_img, true);
						imagedestroy($src_img);
						if (check_picture_type($pixlie_pic_type, "png")) imagepng($dst_img, $pixlie_cache_path, $pixlie_config['png_Quality']);
						elseif (check_picture_type($pixlie_pic_type, "jpg")) imagejpeg($dst_img, $pixlie_cache_path, $pixlie_config['pic_image_quality']);
						elseif (check_picture_type($pixlie_pic_type, "gif")) imagegif($dst_img, $pixlie_cache_path);
	...
	/************************** 8.3.2 Berechne Bilder nach dem Typ "uncut" **************************/
					...
					case 'uncut':
						...
						if (check_picture_type($pixlie_pic_type, "png")) imagesavealpha($dst_img, true);
						imagedestroy($src_img);
						if ($quality_round > 0) unlink($pixlie_cache_path);
						if (check_picture_type($pixlie_pic_type, "png"))imagepng($dst_img, $pixlie_cache_path, $pixlie_config['png_Quality']);
						elseif (check_picture_type($pixlie_pic_type, "jpg")) imagejpeg($dst_img, $pixlie_cache_path, $pixlie_config['pic_image_quality']);
						elseif (check_picture_type($pixlie_pic_type, "gif")) imagegif($dst_img, $pixlie_cache_path);
						$pixlie_cache_name = md5($pixlie_query).'_'.$pixlie_item_picsize.".".$pixlie_pic_type;
	...
	/************************** 8.3.4 Berechne Bilder nach dem Typ "fixed" ************************/					
					...
					case 'fixed':	
						...
						@ini_set('memory_limit', '50M');
						$src_img = create_data($pixlie_pic_type, $pixlie_item_path);
						if ($pixlie_render_size[1] == 'w') {
							if (imagesx($src_img) == imagesy($src_img)) {
								$dst_w = $pixlie_render_size[0];
								$dst_h = $pixlie_render_size[0];
							}
							else {
								$dst_w = round( $pixlie_render_size[0] / imagesx($src_img) * imagesy($src_img));
								$dst_h = $pixlie_render_size[0];
							}
						}
						elseif($pixlie_render_size[1] == 'h') {
							if (imagesx($src_img)==imagesy($src_img)) {
								$dst_w = $pixlie_render_size[0];
								$dst_h = $pixlie_render_size[0];
							}
						...
						else pixlie_error($pixlie_lang['render_missing_option']);
						$dst_img = imagecreatetruecolor($dst_h,$dst_w);
						if (check_picture_type($pixlie_pic_type, "png")) imagealphablending($dst_img, false);
						imagecopyresampled($dst_img,$src_img,0,0,0,0,$dst_h,$dst_w,imagesx($src_img),imagesy($src_img));
						if (check_picture_type($pixlie_pic_type, "png")) imagesavealpha($dst_img, true);
				...
				$new_img = create_data($pixlie_pic_type, $pixlie_cache_path);
				$new_img_x = imagesx($new_img);
				$new_img_y = imagesy($new_img);
				$quality_round++;
				} while ((($old_img_x > $new_img_x) && ($old_img_y > $new_img_y) && ($pixlie_old_size < filesize($pixlie_cache_path))) && $quality_round < 5 && $pixlie_config['autoscale_quality']);
			}
	...
	/*********************************** 8.4 Ausgabe des Bildes *************************************/
			...
			switch ($pixlie_pic_type) {
				...
				case ('gif' || 'GIF'):
					...
					break;
				default:
					pixlie_error($pixlie_lang['invalid_datatype']);
					break;
			}
	...
	/********************************** 9.1 Weiche für Ordner / Bild *********************************/
			...
			while(false !== ($pixlie_item_name = readdir($pixlie_dir_handle))){
				if ($pixlie_item_name != "." && $pixlie_item_name != ".." && !in_array($pixlie_item_name, $pixlie_config['pic_exclude'])) {
					$pixlie_item_path = $pixlie_config['dir_upload'].$pixlie_query.'/'.$pixlie_item_name;
					if((filetype($pixlie_item_path)=='file' ) && check_picture_ending($pixlie_item_path, $pixlie_config)){
	...
	/************************************* 9.2 JPG Grunddaten lesen *********************************/
						...
						$pixlie_path_explode = explode(".", basename($pixlie_item_path));
						$pixlie_pic_type = $pixlie_path_explode[count($pixlie_path_explode)-1];
						$pixlie_table_file[$pixlie_item_key]['name'] =utf8_encode(preg_replace('/\.'.$pixlie_pic_type.'/i','',$pixlie_item_name));
						...
						if ($pixlie_pic_type == "jpeg" || $pixlie_pic_type == "JPEG") //Zuschnitt auf Stringlaenge
	...
	/*********************************** 9.3 IPTC Metadaten lesen ***********************************/
						$pixlie_path_explode = explode(".", basename($pixlie_item_path));
						$pixlie_pic_type = $pixlie_path_explode[count($pixlie_path_explode)-1];
						if(($pixlie_config['show_metadata_iptc']) && (check_picture_type($pixlie_pic_type, 'jpg'))){
							if (!$pixlie_config['use_dims'])
	...
	/*********************************** 9.4 EXIF Metadaten lesen ***********************************/
						...
						if(($pixlie_config['show_metadata_exif']) && (check_picture_type($pixlie_pic_type, 'jpg'))) {
	...
	/*********************************** 9.5 Ordner Grunddaten lesen ********************************/
			...
			$pixlie_table_env['pixlie_version'] = '2.3.2';
			...
			if (!isset($istartpos))
					pixlie_error($pixlie_lang['path_not_exist']);
		}
		else
